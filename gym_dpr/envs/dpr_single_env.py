...
class DPRSingleEnv(gym.Env):
    ...
    def __init__(self, numBots, worldClass=World, botClass=CircularBot, superBotClass=SuperCircularBot,
        ...
        if manipulationTask:
            ...
            # EDITED -> added dtype param to fix check_env() method
            self.observation_space = spaces.Box(low=-1, high=1, shape=(4 * numBots, ), dtype=np.float64)
    ...
    def wavePolicy(self):
        '''
        Hand-crafted baseline algorithm. Particle robots recieve an index based on distance to the goal
    ...
    def step(self, action):
        ...
        if self.manipulationTask == True:
            ...
            # done = bool((self.steps > 5000) or (np.linalg.norm(self.agent.getCOM() - self.goal) < 10))
